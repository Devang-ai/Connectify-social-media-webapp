            ...
            <h3 className="font-semibold text-sm flex items-center flex-wrap gap-x-2">
              ...
              {!isMe && !isFriend && (
                <button 
                  onClick={() => !isRequestSent && handleFollow(authorId)}
                  disabled={isRequestSent}
                  className={`px-2 py-0.5 rounded-full text-[10px] font-bold transition ${isRequestSent ? 'bg-slate-100 dark:bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-primary text-white hover:bg-indigo-600'}`}
                >
                  {isRequestSent ? 'Request Sent' : 'Follow'}
                </button>
              )}
